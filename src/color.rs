...
pub const fn hex_const(hex: &str) -> Color {
     ...
     Color {
         r: div_lut(hex_digit(bytes[1]) * 16 + hex_digit(bytes[2])),
         g: div_lut(hex_digit(bytes[3]) * 16 + hex_digit(bytes[4])),
         b: div_lut(hex_digit(bytes[5]) * 16 + hex_digit(bytes[6])),
         a: 1.0,
     }
}